...
public class Example extends Object
{
	 ...
	 *（Tree-1, Tree-2, Tree-3のウィンドウたち）から1つのモデルを観測している状態を作り出す。
	 * その後、モデルの内容物を先ほどキャプチャした画像にして、
	 * 自分が変化したと騒いだ瞬間、Tree-1, Tree-2, Tree-3のすべてのウィンドウが更新される。
	 * そして、モデルの内容物をnull化して、自分が変化したと騒ぎ、すべてのウィンドウが空に更新される。
	 * この過程を何回か繰り返すことで、Tree: Model-View-Controller（Observerデザインパターン）が
	...
	public static void main(String[] arguments)
	{
		...
		// Treeの出現数から、最初のウィンドウの出現位置を計算する。
		Integer howMany = 3; // Treeの出現回数
		...
		new Condition(() -> index.get() < howMany).whileTrue(() ->
		{
			...
			JFrame aWindow = new JFrame("Tree-" + Integer.toString(index.get() + 1));
